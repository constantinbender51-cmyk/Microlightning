...
lr_live.py (MODIFIED)
Key changes:
1. Import binance_ohlc for training data
2. Use Binance for model.fit() (years of data)
3. Use Kraken for live predictions (real-time data from your trading exchange)
"""
...
import kraken_ohlc
import binance_ohlc  # <-- NEW IMPORT
...
SYMBOL_OHLC_BINANCE = "BTCUSDT"  # <-- NEW: Binance symbol
INTERVAL_KRAKEN = 1440          # Kraken uses minutes
...
# [stoch_rsi, ema, ModelBundle class remain unchanged]
...
# [portfolio_usd, mark_price, order helpers remain unchanged]
...
def place_stop(api: kf.KrakenFuturesApi, side: str, size_btc: float, fill_price: float, pred6_abs: float):
    allowed_move = STOP_FAC * pred6_abs / 100.0
    if side == "buy":
...
def smoke_test(api: kf.KrakenFuturesApi, model6: ModelBundle, model10: ModelBundle):
    ...
    df = kraken_ohlc.get_ohlc(SYMBOL_OHLC_KRAKEN, INTERVAL_KRAKEN)  # Use Kraken for live data
...
def scan_signal(api: kf.KrakenFuturesApi, model6: ModelBundle, model10: ModelBundle) -> str:
    # Use Kraken for LIVE prediction (real-time market data from your exchange)
...
def trade_step(api: kf.KrakenFuturesApi, model6: ModelBundle, model10: ModelBundle):
    ...
    if curr != "HOLD" and curr != prev:
        ...
        if dry:
            ...
            pred6_today = abs(model6.predict_last(kraken_ohlc.get_ohlc(SYMBOL_OHLC_KRAKEN, INTERVAL_KRAKEN)))
...
def main():
    ...
    # ============================================================
    # KEY CHANGE: Use Binance for training (maximum history)
    # ============================================================
    log.info("Fitting models using Binance historical data...")
    ...
    model6.fit(df_train)
    model10.fit(df_train)
    log.info("Models ready (trained on Binance, will predict on Kraken)")
